...
export class FriendRequestsPage implements OnInit {
  ...
  async acceptRequest(request: any) {
    const alert = await this.alertCtrl.create({
      ...
      message: `Do you want to accept connection request from ${request.requested_by_first_name} ${request.requested_by_last_name}?`,
      buttons: [
        {
          text: 'Cancel',
          role: 'cancel'
  ...
  async rejectRequest(request: any) {
    const alert = await this.alertCtrl.create({
      ...
      message: `Reject connection request from ${request.requested_by_first_name} ${request.requested_by_last_name}?`,
      buttons: [
        {
          text: 'Cancel',
          role: 'cancel'
